    ...
    if (rank == 0){
      ...
      if (norm == 0){
        for (i = 1; i < procs; i++){
          ...
          for (row = norm + 1 + i; row < N; row += procs){
            MPI_Send(&A[row][norm], N - norm, MPI_FLOAT, i, 0, MPI_COMM_WORLD);
            // printf("stuck\n");
      ...
      if (norm == 0){
        ...
        for (row = norm + 1 + rank; row < N; row += procs){
          MPI_Recv(&A[row][norm], N - norm, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, &status1);
          // printf("stuck!\n");
      ...
      if(norm % procs == rank){
        MPI_Bcast(&A[norm + 1 + rank], N, MPI_FLOAT, 0, MPI_COMM_WORLD);      
        MPI_Bcast(&B[norm + 1 + rank], 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
      }
    }
