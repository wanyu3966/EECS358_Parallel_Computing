...
void gauss() {
  ...
  for (norm = 0; norm < N - 1; norm++){
    ...
    if (rank == 0){
      ...
      if(norm % procs != 0){
        MPI_Recv(&A[norm + 1], N, MPI_FLOAT, norm % procs, 0, MPI_COMM_WORLD, &status3);
        MPI_Recv(&B[norm + 1], 1, MPI_FLOAT, norm % procs, 0, MPI_COMM_WORLD, &status4);
      }
        ...
        if(norm % procs == rank){
          for(k = 0; k <procs && k != 0; k++){
             count2++;
            printf("count2%d\n", count2);
            MPI_Send(&A[norm + 1], N, MPI_FLOAT, k, 0, MPI_COMM_WORLD);  
           
            MPI_Send(&B[norm + 1], 1, MPI_FLOAT, k, 0, MPI_COMM_WORLD);
          }          
        }
