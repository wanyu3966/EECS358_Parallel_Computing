...
void gauss() {
  ...
  for (norm = 0; norm < N - 1; norm++){
    // Send the norm being processed to all processors
    MPI_Bcast(&norm, 1, MPI_INT, 0, MPI_COMM_WORLD);
    // Send the required value of row to all processors
    ...
    if (rank == 0){
      ...
      // receive the data computed from other processors
      for (i = 0; i < procs; i++){
        //apply static interleaved scheduling and assign data to related processors
        for (row = norm + 1 + i; row < N; row += procs){
          MPI_Recv(&A[row], N, MPI_FLOAT, i, 1, MPI_COMM_WORLD, &status1);
          MPI_Recv(&B[row],1,MPI_FLOAT,i,1,MPI_COMM_WORLD,&status1);
        }
      }
    }else{
