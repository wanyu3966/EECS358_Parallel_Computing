 ...
 */
void gauss() {
  int norm, row, col, i, k;  /* Normalization row, and zeroing
      * element row and col */
  float multiplier;
  MPI_Status status;
  ...
  if (rank == 0){
    for (i = 1; i < procs; i++){
        ...
        for (row = i; row < N; row += procs){
          ...
          // printf("stuck\n");
          MPI_Recv(&B[row], 1, MPI_FLOAT, 0, 1, MPI_COMM_WORLD, &status2);
        }
  ...
  for (norm = 0; norm < N - 1; norm++){
     ...
     if (norm % procs == rank){
      for(k = 0; k < procs; k++){
        if (k != rank){
          MPI_Send(&A[norm], N, MPI_FLOAT, k, 0, MPI_COMM_WORLD);
          MPI_Send(&B[norm], 1, MPI_FLOAT, k, 0, MPI_COMM_WORLD);
        }      
      }
    } else {
      for(k = 0; k < procs; k++){
        if (k != rank){
          MPI_Recv(&A[norm], N, MPI_FLOAT, k, 0, MPI_COMM_WORLD, &status);
          MPI_Recv(&B[norm], 1, MPI_FLOAT, k, 0, MPI_COMM_WORLD, &status);
        }      
      }
    }
    //compute its data
    for (row = rank; row < N; row += procs) {
      if (row != 0 && row>norm){
    ...
    if (norm = N - 2){
      int rank_final = norm % procs + 1;
      MPI_Bcast(&A[norm + 1], N, MPI_FLOAT, rank_final, MPI_COMM_WORLD);    
    }
